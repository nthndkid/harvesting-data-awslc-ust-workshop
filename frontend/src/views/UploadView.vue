...
async function handleSubmit() {
  ...
  // Basic URL validation if provided — backend handles "google.com" -> "https://google.com"
  if (demoUrl.value.trim()) {
    const url = demoUrl.value.trim()
    // Minimal check to ensure it looks like a domain or URL
    if (!url.includes('.') || url.length < 4) {
      submitError.value = 'PLEASE PROVIDE A VALID DEMO URL (E.G. GOOGLE.COM)'
      return
    }
  }
  ...
  try {
    ...
    const pdfKey = await uploadStore.uploadPdf(pdfFile.value as File)
